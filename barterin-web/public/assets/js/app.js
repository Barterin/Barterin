    ...
    return new Promise((resolve, reject) => {
        ...
        if ("js" == format) {
            ...
            return (
                ...
                keep == false && script.setAttribute("temp", ""),
                (script.src = `${url}${
                    keep == false ? `?${+new Date()}` : ""
                }`),
        ...
        if ("css" == format) {
            ...
            (link.rel = "stylesheet"),
                keep == false && link.setAttribute("temp", ""),
                (link.type = "text/css"),
                (link.href = url),
...
const loadFiles = function (arrayJs, keep = false) {
    return new Promise((resolve, reject) => {
        ...
        arrayJs.forEach(async function (url) {
            await loadFile(url, keep),
                current++,
                total == current && resolve(!0);
        });
    });
};
...
const baseUrl = document
    ...
    .getAttribute("content");
const environmentStatus = document
    .querySelector(`meta[name="environment"]`)
    .getAttribute("content");
__access_token = document
...
const detectLoadJs = function (source = document) {
    ...
    nodes.forEach((value, index) => {
        // console.log(value)
        loadFile(
            `/assets/js/page/${value.localName.split("-")[1]}${
                environmentStatus == "production" ? ".min" : ""
            }.js`
        );
        value.remove();
    });
...
const detectLoadCSS = function (source = document) {
    ...
    nodes.forEach((value, index) => {
        // console.log(value)
        loadFile(
            `/assets/css/page/${value.localName.split("-")[1]}${
                environmentStatus == "production" ? ".min" : ""
            }.css`
        );
        value.remove();
...
const msgSweetSuccess = function (
    pesan,
    options = {
        title: "Sukses",
        timer: 3000,
...
const msgSweetWarning = function (
    pesan,
    options = {
        title: "Peringatan",
        timer: 3000,
