const loadFile = function (url) {
    return new Promise((resolve, reject) => {
        ...
        return script.setAttribute("defer", ""), script.src = url, script.addEventListener("load", (function () {
            resolve(!0)
        })), !__loadedJs.includes(url) && document.head.appendChild(script), __loadedJs.push(url), 1
    })
};
let __loadedJs = [];
...
function sendMessage() {
    ...
    document.getElementById('chatParent').scrollTo(0, document.getElementById('chatParent').scrollHeight);
    sc.emit('sendMessage', {
        sender: userId,
...
loadJs([
    ...
    $(document).ready(function () {
        ...
        sc.on('targetTyping', (data) => {
            $(`#typingIndicator[data-id="${data.sender}"]`).text(`${data.sender} mengetik . . .`)
            $(`#typingIndicator[data-id="${data.sender}"]`).removeClass('d-none')
        })
